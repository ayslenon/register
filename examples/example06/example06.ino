/*
  ...
  [x]Example 06 - Simple "digitalRead" to the register
...
void loop() {
  // in this example, we are going to see whats inside an address of a register
  // we can read from addr 0 to 7
  for (int i = 0; i < 8; i++){
    addrState = registerTest.addrState(addr);
    Serial.print("The value on the ");
    Serial.print(i);
    Serial.print(" addr is: ");
    Serial.println(addrState);
    delay(100);
    // a 0 means that the output should be off, and 1 means that the output should be on
  }
