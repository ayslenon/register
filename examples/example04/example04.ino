/*
  ...
  [x]Example 04 - How to reset the register
...
void loop() {
  ...
  // here we will se how easy is to clean all the bits on the register
  // we have 2 ways to do it:
  // from the RST pin on IC 74HC595, setting to 0, and 1 again
  // or passing 0 to setAllValues
  // if you're using the 5 pin way, you can to the first method, but in both methods
  // you can call the same function

  registerTest.resetRegister();
  delay(1000);
  // if you're using a 5 pin mode, it resets from IC, otherwise it you call internally setAllOutputs(0)
}
